...
B_Direction = checkDirection(outPath)
B0 = checkB0(outPath)
eta = checkEta(outPath)
...
print "Sampling with a radius of",galaxySampleRadius,"for galaxies and",haloSampleRadius,"for halos"

sys.stdout.flush()

indicesInCone = {}
with h5py.File(os.path.expanduser(datPath+'indicesInCone_'+str(numCube)+'_PARTICLE_0'),'r') as data:
    for i in data.keys():
        indicesInCone[i] = data[i][:]
with h5py.File(os.path.expanduser(datPath+'indicesInCone_'+str(numCube)+'GALAXY'),'r') as data:
    for i in data.keys():
        indicesInCone[i] = data[i][:]
with h5py.File(os.path.expanduser(datPath+'indicesInCone_'+str(numCube)+'HALO'),'r') as data:
    for i in data.keys():
        indicesInCone[i] = data[i][:]
...
#===============================================================
dlDict = {}
with h5py.File(os.path.expanduser('directionOfField_'+coherenceScale+'/Bdirection_'+str(numCube)+'_'+str(sectionOfRays)+'_'+coherenceScale+'kpc'),'r') as data:
...
#===============================================================
#nearParticlesDict = {}
particlePositions,particleAttribs = readLightcone('PARTICLE',indicesInCone,datPath,numCube)
...
for i in range(len(particleAttribs.keys())):
    print particleAttribs.keys()[i],len(particleAttribs.values()[i]),'eg:',particleAttribs.values()[i][0]
...
#					  READ IN GALAXY DATA
#===============================================================
galaxyPositions,galaxyAttribs, galaxyGlist = readLightcone('GALAXY',indicesInCone,datPath,numCube)
for i in range(len(galaxyAttribs.keys())):
    print 'Galaxy Attributes'
    print galaxyAttribs.keys()[i],len(galaxyAttribs.values()[i]),galaxyAttribs.values()[i][0]

#===============================================================
#                       READ IN HALO DATA
#===============================================================
haloPositions,haloAttribs, haloGlist = readLightcone('HALO',indicesInCone,datPath,numCube)
for i in range(len(haloAttribs.keys())):
    print 'Halo Attributes'
    print haloAttribs.keys()[i],len(haloAttribs.values()[i]),haloAttribs.values()[i][0]
...
particleID,galaxyID,haloID,particleDist,galaxyDist,haloDist = readStructure(numCube,sectionOfRays,inPath,numSources)
...
zrFunc = sip.UnivariateSpline(map(float, zip(*points)[1]), map(float, zip(*points)[2]), s=0)
...
for i in particleID.keys():
        proc +=1
        sourcekey = str(i)
        if len(particleID[sourcekey])==0: #these rays were too short to be in the current section so we can ignore them
            print 'Processing ray',i,'which is empty'
            ignoreKey = np.array([])  #Dont want to waste memory on the rays that are too long
            ...
            returned = obtainRM(particleID[sourcekey],galaxyID[sourcekey],haloID[sourcekey],particleDist[sourcekey],galaxyDist[sourcekey],haloDist[sourcekey],particleAttribs,B0,eta,pScale,alpha,dlDict[sourcekey],B_Direction,LA,LA_cut,LO,galaxyAttribs,galaxyGlist,haloAttribs,haloGlist,indicesInCone,out_q)
            rmVec = returned[0]
...
cubeDistanceRadii = path.join(dir, outPath+'cubeDistanceRadii'+str(numCube)+'_'+str(sectionOfRays))
rotationMeasureFile = path.join(dir, outPath+'rotationMeasure'+str(numCube)+'_'+str(sectionOfRays))
particles_galaxies_halos_hitPerSourceFile = path.join(dir, outPath+'particles_galaxies_halos_hitPerSource_'+str(numCube)+'_'+str(sectionOfRays))
NeBFile = path.join(dir, outPath+'NeBprofiles'+str(numCube)+'_'+str(sectionOfRays))

print 'Files allocated'
sys.stdout.flush()

stringToWrite = str(numCube)+','+str(np.max(particleAttribs['redshift_gas']))+','+str(np.max(particleAttribs['distance_gas']))+','+str(np.max(galaxyAttribs['radii_r200']))+','+str(np.max(haloAttribs['radii_r200']))+'\n'
...
with h5py.File(particles_galaxies_halos_hitPerSourceFile) as hf:
    hf.create_dataset(str(numCube), data =numberHit, chunks = True, compression = 'gzip',compression_opts=4)
...
with h5py.File(rotationMeasureFile) as hf:
        ...
        for i in range(len(rotationMeasure)):
            grp.create_dataset(str(i), data =rotationMeasure[i], chunks = True, compression = 'gzip',compression_opts=4)
        grp = hf.create_group('rotationMeasure_galaxy')
        for i in range(len(galaxyRM)):
            grp.create_dataset(str(i), data =galaxyRM[i], chunks = True, compression = 'gzip',compression_opts=4)
        grp = hf.create_group('rotationMeasure_halo')
        for i in range(len(haloRM)):
            grp.create_dataset(str(i), data =haloRM[i], chunks = True, compression = 'gzip',compression_opts=4)

print 'lengths B,Ne', len(B),len(Ne)
with h5py.File(NeBFile) as hf:
    grp = hf.create_group('B')
    for i in range(len(B)):
        grp.create_dataset(str(i), data =B[i], chunks = True, compression = 'gzip',compression_opts=4)
    grp = hf.create_group('Ne')
    for i in range(len(Ne)):
        grp.create_dataset(str(i), data =Ne[i], chunks = True, compression = 'gzip',compression_opts=4)
    grp = hf.create_group('dl')
    for i in range(len(dl)):
        grp.create_dataset(str(i), data =dl[i], chunks = True, compression = 'gzip',compression_opts=4)
