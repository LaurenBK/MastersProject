...
script_path = "/mnt/lustre/users/lhunt/CurrentCode/structureToRM.py"
...
# check params
params()
outPath = outPath[:-1] + direction + '_' + str(B0) + '_' + str(exponent) + \
          '_' + str(coherenceScale)
...
if rank == 0:
    for i in tasklist:
        ...
        if not os.path.exists(
                outPath + '/rotationMeasure' + str(cubeToRun) + '_' +
                str(raySectionToRun)):
            print 'With starting cube:', startCube, 'we process cube', \
                cubeToRun, 'with subsection', raySectionToRun
            logfile = os.path.join(
                dir, outPath + '/logs/tracing_' + direction + '_' +
                str(cubeToRun) + '_' + str(raySectionToRun) + '.log')
            exec_command = "python " + script_path + " " + str(cubeToRun) + \
                           ' ' + str(raySectionToRun) + ' ' + outPath + '/ ' +\
                           coherenceScale + " > " + logfile
            os.system(exec_command)
        else:
            print 'rotationMeasure' + str(cubeToRun) + '_' + \
                  str(raySectionToRun), 'exists, so skipping this one'
else:
    for i in tasklist:
        cubeToRun = startCube + (i - startCube) / raypieces
        raySectionToRun = (i - startCube) % raypieces
        if not os.path.exists(outPath + '/rotationMeasure' + str(cubeToRun) +
                              '_' + str(raySectionToRun)):
            print 'With starting cube:', startCube, 'we process cube', \
                cubeToRun, 'with subsection', raySectionToRun
            logfile = os.path.join(dir, outPath + '/logs/tracing_' + direction
                                   + '_' + str(cubeToRun) + '_' +
